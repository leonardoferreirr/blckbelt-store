    ...
    <div className="lg:grid lg:grid-cols-[220px_1fr] lg:gap-10">
      ...
      <aside className="mb-8 lg:mb-0">
        <div className="lg:sticky lg:top-32">
          <p className="font-display text-sm font-semibold uppercase tracking-widest">Filtrar</p>

          <div className="mt-5">
            <p className="text-xs font-medium uppercase tracking-widest text-muted">Tamanho</p>
          ...
          <label className="mt-6 flex cursor-pointer items-center gap-2 text-sm">
            <input type="checkbox" checked={onlySale} onChange={(e) => setOnlySale(e.target.checked)} className="accent-accent" />
            Só promoções
          </label>

          {(sizes.length > 0 || onlySale) && (
            <button onClick={() => { setSizes([]); setOnlySale(false); }} className="mt-4 text-xs text-muted underline hover:text-accent">
              Limpar filtros
            </button>
          )}
        </div>
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          <p className="py-20 text-center text-muted">Nenhuma peça com esse filtro.</p>
        ) : (
