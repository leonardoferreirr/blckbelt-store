...
export default function ProductDetail({ product }: { product: Product }) {
  ...
  const installment = formatPrice(Math.round(product.price / 6));
  ...
  const ACCORDIONS = [
    { t: "Descrição", key: "desc" },
    { t: "Composição e cuidados", key: "comp" },
    { t: "Trocas e devoluções", key: "ship" },
  ];
  ...
  return (
    <div className="lg:grid lg:grid-cols-2 lg:gap-12">
      ...
      <div className="mt-8 lg:mt-0">
        ...
        <p className="mt-1 text-sm text-muted">ou 6x de {installment} sem juros</p>

        {/* tamanho */}
        <div className="mt-8">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium uppercase tracking-widest">Tamanho</span>
            <button className="text-xs text-muted underline hover:text-accent">Guia de medidas</button>
          </div>
          ...
          {err && <p className="mt-2 text-xs text-sale">Selecione um tamanho.</p>}
        </div>

        {/* add */}
        <button
          onClick={handleAdd}
          className="mt-6 w-full bg-ink py-4 text-sm font-semibold uppercase tracking-widest text-paper hover:bg-accent transition-colors"
        >
          Adicionar à sacola
        </button>
            ...
            <div key={a.key} className="border-b border-line">
              ...
              {openAcc === a.key && (
                <div className="pb-5 text-sm leading-relaxed text-muted">
                  ...
                  {a.key === "ship" && "Você tem até 30 dias para trocar ou devolver. Primeira troca por tamanho é grátis. Envio para todo o Brasil."}
                </div>
              )}
            </div>
          ))}
