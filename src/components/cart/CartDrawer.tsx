      ...
      >
        <header className="flex items-center justify-between border-b border-line px-5 py-4">
          <h2 className="font-display text-lg uppercase tracking-tight">
            Sacola {count > 0 && <span className="text-muted">({count})</span>}
          </h2>
          <button onClick={close} aria-label="Fechar" className="text-2xl leading-none text-ink hover:text-accent">
            &times;
          </button>
        </header>
          ...
          <>
            ...
            <div className="border-b border-line px-5 py-3">
              <p className="text-xs text-ink">
                {missing > 0 ? (
                  <>Faltam <strong>{formatPrice(missing)}</strong> para o frete grátis</>
                ) : (
              ...
              {items.map((it) => (
                <div key={`${it.id}-${it.size}`} className="flex gap-4 border-b border-line py-4">
                  ...
                  <div className="flex flex-1 flex-col">
                    <Link href={`/produto/${it.slug}`} onClick={close} className="font-display text-xs uppercase tracking-tight hover:text-accent">
                      {it.name}
                    </Link>
                    <span className="mt-0.5 text-xs text-muted">Tam: {it.size}</span>
            ...
            <footer className="border-t border-line px-5 py-4">
              ...
              <p className="pt-2 text-center text-[11px] text-muted">
                Checkout e pagamento a plugar (Stripe / Mercado Pago)
              </p>
            </footer>
          </>
        )}
